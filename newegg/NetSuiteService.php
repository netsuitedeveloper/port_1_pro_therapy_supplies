...
class NetSuite_Service
{
    ...
    public static function update($record, $retry = 0)
    {
        try {
            ...
            if (!$response->writeResponse->status->isSuccess) {
                throw new Exception('Unable to get: ' . print_r($response->writeResponse->status->statusDetail, true));
            }
            return $response;
        } catch(Exception $ex) {
            Log::ex($ex, 'update');
    ...
    public static function get($type, $internalId, $retry = 0)
    {
        try {
            ...
            if (!$response->readResponse->status->isSuccess) {
                throw new Exception('Unable to get: ' . print_r($response->readResponse->status->statusDetail, true));
            }
            return $response;
        } catch(Exception $ex) {
            Log::ex($ex, 'get');
    ...
    public static function add($record, $retry = 0) 
    {
        try {
            ...
            if (!$response->writeResponse->status->isSuccess) {
                throw new Exception('Unable to add record: ' . print_r($response->writeResponse->status->statusDetail, true));
            }
